...
async function us(e, r, o) {
  ...
  if (typeof window < "u") {
    const c = window;
    typeof n == "string" && n !== "" ? c.__SURVEYJS_LICENSE_KEY = n : delete c.__SURVEYJS_LICENSE_KEY;
  }
...
`).split(/\n{2,}/).map((o) => {
    const a = o.trim();
    return a === "" ? "" : /^(- |\* )/.test(a) ? `<ul>${a.split(/\n/).filter((n) => /^(- |\* )/.test(n)).map((n) => `<li>${ur(n.replace(/^(- |\* )/, ""))}</li>`).join("")}</ul>` : `<p>${ur(a).replace(/\n/g, "<br>")}</p>`;
  }).join("") : "";
}
function zs({
...
function Ms(e, r, o) {
  ...
  const t = Number(o.getPropertyValue("startTime") ?? 0), n = Number(o.getPropertyValue("endTime") ?? 0), i = !!(o.getPropertyValue("autoStart") ?? !1), c = String(o.getPropertyValue("requiredWatchMessage") ?? "").trim(), d = !!o.isRequired;
  r.innerHTML = "";
  const l = document.createElement("video");
  l.src = a, l.controls = !o.readOnly, l.preload = "metadata", !o.readOnly && i && (l.autoplay = !0, l.muted = !0), r.appendChild(l);
  const g = [];
  let p = t || 0, m = !d || n <= t;
  ...
  const f = (e.locale || "en").slice(0, 2), b = c !== "" ? c : pr[f] ?? pr.en ?? "", k = () => {
    !d || m || typeof window < "u" && b && alert(b);
  };
...
function $s(e, r = 100) {
  ...
  })).filter((c) => Number.isFinite(c.lat) && Number.isFinite(c.lon)), n = As(t, r);
  let i = 0;
  for (let c = 1; c < t.length; c++) {
    const d = t[c - 1], l = t[c];
    d && l && (i += Ns(d, l));
  }
  return { sampledPoints: n, distanceMeters: i, pointCount: t.length };
}
function As(e, r) {
  if (e.length <= r) return e.slice();
  const o = (e.length - 1) / (r - 1), a = [];
  for (let t = 0; t < r; t++) {
    const n = Math.round(t * o), i = e[n];
    i !== void 0 && a.push(i);
  }
  return a;
}
function Ns(e, r) {
  const a = Ae(r.lat - e.lat), t = Ae(r.lon - e.lon), n = Ae(e.lat), i = Ae(r.lat), c = Math.sin(a / 2) ** 2 + Math.cos(n) * Math.cos(i) * Math.sin(t / 2) ** 2;
...
async function Ds(e, r, o) {
  ...
  if (n.className = "sh2-gpx-question__meta", !r.readOnly) {
    ...
    c.type = "file", c.accept = ".gpx,application/gpx+xml,application/xml", a.appendChild(c), c.addEventListener("change", async () => {
      ...
      if (d)
        try {
          ...
          n.textContent = `Failed to parse GPX: ${l.message}`;
        }
    });
  }
  ...
  if (i?.sampledPoints?.length) {
    const c = {
      sampledPoints: i.sampledPoints.map((d) => ({ lat: d.lat, lon: d.lon })),
      distanceMeters: Number(i.distanceMeters ?? 0),
      pointCount: i.sampledPoints.length
    };
    gr(t, c), br(n, c);
  }
}
...
async function Fs(e, r, o) {
  ...
  n.controls = !0, n.preload = "metadata";
  const i = document.createElement("button");
  i.type = "button", i.textContent = "Record", i.disabled = r.readOnly;
  const c = document.createElement("button");
  c.type = "button", c.textContent = "Stop", c.disabled = !0, a.appendChild(i), a.appendChild(c), a.appendChild(t), a.appendChild(n);
  ...
  i.addEventListener("click", async () => {
    ...
    try {
      ...
      t.textContent = `Microphone unavailable: ${b.message}`;
      return;
    }
    d = new MediaRecorder(l, { mimeType: "audio/webm" }), d.addEventListener("dataavailable", (b) => {
      b.data.size > 0 && g.push(b.data);
    }), d.addEventListener("stop", async () => {
      const b = new Blob(g, { type: d?.mimeType ?? "audio/webm" }), k = Date.now() - p;
      l?.getTracks().forEach((v) => v.stop()), l = null, d = null;
      const C = `recording-${Date.now()}.webm`, E = new File([b], C, { type: b.type });
      try {
        ...
        o.setAnswer({
          ...
          durationMs: k,
          mimeType: b.type
        }), n.src = v.downloadUrl, t.textContent = `Recorded ${(k / 1e3).toFixed(1)} s.`;
      } catch (v) {
        t.textContent = `Upload failed: ${v.message}`;
      }
    }), d.start(), p = Date.now(), i.disabled = !0, c.disabled = !1, t.textContent = "Recording…", m > 0 && window.setTimeout(() => {
      d && d.state === "recording" && d.stop();
    }, m * 1e3);
  }), c.addEventListener("click", () => {
    d && d.state === "recording" && d.stop(), c.disabled = !0, i.disabled = r.readOnly;
  });
  const f = r.value;
  (f?.url ?? f?.downloadUrl) && (n.src = f.downloadUrl ?? f.url);
}
let fr = !1;
async function Dr(e) {
  if (fr)
...
function Os(e, r) {
  e.onAfterRenderQuestion.add((o, a) => {
    const t = a.question, n = t.getType(), i = (c) => {
      const d = o;
    ...
    const n = r.responseIdProvider(), i = a.question, c = a.name ?? i?.name ?? "file";
    ...
    const t = a.content, n = typeof t == "string" ? t : t?.url ?? t?.content;
    if (typeof n == "string" && n !== "") {
      a.callback("success", n);
      return;
    }
    a.callback("error");
  }), e.onClearFiles.add((o, a) => {
    const t = a.value, n = [], i = (c) => {
      if (c && typeof c == "object" && "id" in c) {
        const d = c.id;
        typeof d == "number" && Number.isFinite(d) && n.push(d);
    ...
    if (Array.isArray(t) ? t.forEach((c) => i(c)) : i(t), n.length === 0) {
      a.callback("success");
      return;
    }
    Promise.all(n.map((c) => r.deleteFile(c).catch(() => {
...
function Or() {
  ...
  return ae(() => {
    const o = r === "dark", a = e.primaryColor, t = e.colors[a] ?? e.colors.blue, n = e.colors.gray, i = e.colors.dark;
    return {
      mode: o ? "dark" : "light",
      primary: t[6],
      primaryText: e.white,
      primarySoft: o ? t[8] : t[1],
      background: o ? i[7] : e.white,
      surface: o ? i[6] : e.white,
      surfaceMuted: o ? i[5] : n[0],
      text: o ? i[0] : n[9],
      border: o ? i[4] : n[3]
    };
  }, [r, e]);
...
function qs({
  ...
  const t = r ?? e, n = ae(() => ie(t, "survey-js"), [t]), i = ae(() => ie(t, "survey-js-theme"), [t]), c = ae(() => oo(t), [t]), d = Or(), [l, g] = j(null), [p, m] = j(null), [f, b] = j({ kind: "loading" }), [k, C] = j(null), E = se(null), v = se(null), D = se(null), S = se(null), O = se(!1), V = se(null);
  R(() => {
    let _ = !1;
    return (async () => {
      try {
        ...
        _ || b({ kind: "error", message: `SurveyJS runtime failed to load: ${h.message}` });
      }
    })(), () => {
      _ = !0;
    };
    ...
    return us(n, Zs(c), h).then((z) => {
      ...
      _ || b({ kind: "error", message: `Survey not available: ${z.message}` });
    }), () => {
      _ = !0;
    };
  }, [n, c]), R(() => {
    if (!l || !p || f.kind !== "ready") return;
    let _ = !1;
    const h = new l.Model(p.definition);
    h.applyTheme(
      Ur(
        i ?? p.themeCode ?? "default",
        d
      )
    );
    ...
    return F > 0 && (G = setInterval(() => {
      ...
      ps(p.surveyId, A.data, ee).then((X) => {
        _ || (O.current = !1, D.current = X.responseId, E.current && E.current.clearAll(), c.oncePerUser || c.oncePerSchedule ? b({
          kind: "locked",
  ...
  if (f.kind === "error")
    return /* @__PURE__ */ s(B, { color: "red", title: "Survey unavailable", children: f.message });
  if (f.kind === "not-active")
    return Ke(
      f.label,
      "gray"
    );
...
async function Ws(e, r) {
  const { published: o, config: a, draftStore: t, editModeResponseId: n, assignResponseId: i, captureMeta: c } = r;
  if (c(Nr()), n) {
    const l = await ms(o.surveyId, n).catch(() => null);
    if (l) {
      i(l.responseId), e.data = { ...l.answers };
      return;
    }
  }
  ...
  d ? (i(d.responseId), e.data = { ...d.data }, Number.isInteger(d.pageNo) && d.pageNo >= 0 && (e.currentPageNo = d.pageNo)) : i(`R_${Qe(8)}`);
}
function Qs(e) {
  const r = e.data;
...
function Jr(e, r) {
  ...
  n.setHours(o.hour, o.minute, 0, 0);
  const i = new Date(t);
  return i.setHours(a.hour, a.minute, 0, 0), n.getTime() > i.getTime() && (i.getTime() > t.getTime() ? n.setDate(n.getDate() - 1) : i.setDate(i.getDate() + 1)), {
    start: n.toISOString(),
    end: i.toISOString()
  };
}
...
async function so(e, r, o) {
  try {
    o && r.setLicenseKey(o);
    const a = await import("./survey.pdf-C35MXLTc.js").then((c) => c.s), t = e, n = t.jsonObj ?? t.toJSON?.(), i = new a.SurveyPDF(n, void 0);
    i.data = e.data, i.save(`survey-${Date.now()}.pdf`);
  } catch {
...
function to({ style: e, section: r, styleProps: o, cssClass: a }) {
  const n = no(r ?? e), i = se(null);
  return R(() => {
    if (!n) return;
    let c = null, d = !1;
    return (async () => {
      const g = await import("leaflet").catch((f) => (console.warn("[gpxMap] leaflet not available", f), null));
      if (!g || d || !i.current) return;
      const p = g.default ?? g, m = p.map(i.current).setView([46.95, 7.45], 13);
      c = m, p.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        maxZoom: 19,
        attribution: "© OpenStreetMap contributors"
      }).addTo(m);
      try {
        const f = await fetch(n).then((k) => k.text()), b = io(f);
        if (b.length > 0) {
          const k = p.polyline(b, { color: "#228be6" }).addTo(m);
          m.fitBounds(k.getBounds());
        }
      } catch (f) {
        console.warn("[gpxMap] failed to load GPX", f);
      }
    })(), () => {
      d = !0, c && c.remove();
    };
...
function io(e) {
  ...
  for (let n = 0; n < t.length; n++) {
    const i = Number(t[n]?.getAttribute("lat")), c = Number(t[n]?.getAttribute("lon"));
    !Number.isNaN(i) && !Number.isNaN(c) && a.push([i, c]);
  }
  return a;
}
var lo = {
...
const U = (e, r, o, a) => {
  const t = ss(
    ({ color: n = "currentColor", size: i = 24, stroke: c = 2, title: d, className: l, children: g, ...p }, m) => He(
      "svg",
      {
        ref: m,
        ...lo[e],
...
function Xo(e, r) {
  ...
  if (a)
    for (const t of Qo) {
      const n = {
        fit: (i) => i?.name === t.name && (i.type === "string" || i.type === "text" || i.type === void 0),
        getJSON: () => ({
          type: "comment",
          rows: 6
        }),
        onCreated: (i, c, d) => {
          queueMicrotask(() => {
            ...
            const f = {
              ...
              document.body.contains(m) || (b.destroy(), k.disconnect());
            });
            k.observe(document.body, { childList: !0, subtree: !0 });
          });
        }
      };
      e.PropertyGridEditorCollection.register(n);
    }
}
function Ee(e) {
...
function Le(e, r) {
  const o = e ?? {}, a = r ?? {}, t = [], n = Array.isArray(o.pages) ? o.pages : [], i = Array.isArray(a.pages) ? a.pages : [], c = /* @__PURE__ */ new Map();
  n.forEach((m, f) => {
    c.set(Mr(m, f), { page: m, index: f });
  });
  const d = /* @__PURE__ */ new Map();
  i.forEach((m, f) => {
    d.set(Mr(m, f), { page: m, index: f });
  });
  for (const [m, { page: f, index: b }] of c.entries())
    d.has(m) || t.push({
      kind: "removed",
      path: `pages[${b}]`,
      label: qe(f, b),
  ...
  for (const [m, { page: f, index: b }] of d.entries()) {
    const k = c.get(m);
    if (!k) {
      t.push({
        kind: "added",
    ...
    k.index !== b && t.push({
      ...
      path: `pages[${b}]`,
      label: qe(f, b),
      detail: `page moved from #${k.index + 1} to #${b + 1}`
    }), Zo(k.page, f, b, t);
  }
...
function Zo(e, r, o, a) {
  const t = Array.isArray(e.elements) ? e.elements : [], n = Array.isArray(r.elements) ? r.elements : [], i = /* @__PURE__ */ new Map();
  t.forEach((d, l) => i.set(Pr(d, l), { question: d, index: l }));
  const c = /* @__PURE__ */ new Map();
  n.forEach((d, l) => c.set(Pr(d, l), { question: d, index: l }));
  for (const [d, { question: l, index: g }] of i.entries())
    c.has(d) || a.push({
      kind: "removed",
...
function sa({ surveyId: e, onSurveyChanged: r } = {}) {
  const [o, a] = j(null), [t, n] = j(null), [i, c] = j(null), [d, l] = j(null), [g, p] = j(null), [m, f] = j(null), [b, k] = j(!1), [C, E] = j(!1), [v, D] = j({}), [S, O] = j(!1), [V, T] = j(0), [q, _] = j(!1), [h, z] = j(""), [F, G] = j(!1), Y = se(null), je = Or(), A = t?.themeCode ?? "default", ee = sr({
  ...
  R(() => {
    ...
    if (!e) {
      n(null), c(null), O(!1), T(0), p(null);
      return;
    }
    let w = !1;
    return n(null), c(null), O(!1), T(0), p(null), xe(e).then(($) => {
      w || (n($), r?.($));
  ...
  return /* @__PURE__ */ u(I, { gap: "sm", children: [
    ...
    /* @__PURE__ */ s(ye, { withBorder: !0, p: 0, className: "surveyjs-creator-shell", children: /* @__PURE__ */ s(Zr, { creator: i }) })
  ] });
}
function oa({ surveyId: e } = {}) {
  const [r, o] = j(null), [a, t] = j(0), [n, i] = j(null), [c, d] = j(!1), [l, g] = j(null), [p, m] = j(""), [f, b] = j(null), k = sr({
    pluginId: "sh2-shp-survey-js",
  ...
  const E = L(async () => {
    if (!(!e || !f)) {
      d(!0);
      try {
        await Go(e, f.responseId), b(null), await C();
      } catch (S) {
        i(S.message);
      } finally {
        d(!1);
  ...
  return e ? n ? /* @__PURE__ */ s(B, { color: "red", title: "Could not load responses", children: n }) : r === null ? /* @__PURE__ */ u(M, { gap: "xs", justify: "center", py: "md", children: [
    ...
    v.length === 0 ? /* @__PURE__ */ s(le, { withBorder: !0, p: "lg", children: /* @__PURE__ */ u(I, { gap: "xs", align: "center", children: [
      ...
      /* @__PURE__ */ s(y.Tbody, { children: v.map((S) => /* @__PURE__ */ u(
        ...
        {
          onClick: () => {
            Wo(e, S.responseId).then(g).catch((O) => i(O.message));
          },
      ...
      Te,
      {
        ...
        children: /* @__PURE__ */ u(I, { gap: "sm", children: [
          /* @__PURE__ */ u(x, { size: "sm", children: [
            "Deleting response",
            " ",
            /* @__PURE__ */ s(x, { component: "span", ff: "monospace", children: f?.responseId }),
            " ",
          ...
          /* @__PURE__ */ u(M, { justify: "flex-end", gap: "xs", children: [
            /* @__PURE__ */ s(J, { variant: "default", onClick: () => b(null), disabled: c, children: "Cancel" }),
...
function ta({ surveyId: e, results: r, onSelectResponse: o }) {
  const a = se(null), [t, n] = j(null), [i, c] = j(!1), d = se(null), l = `sh2-shp-survey-js:dashboard:${e}`, g = ae(() => na(r), [r]);
  return R(() => {
    let p = !1;
    return aa().then((m) => {
      if (!p) {
    ...
    const p = da(l), m = ia(g, p), f = (k) => {
      ua(l, k.map((C) => String(C.field ?? "")));
    }, b = new t.Tabulator(a.current, {
      data: r.rows,
      columns: m,
    ...
    return d.current = b, () => {
      b.destroy(), d.current = null;
    };
  }, [t]), R(() => {
    d.current && d.current.replaceData(r.rows);
  }, [r]), i ? /* @__PURE__ */ s(pa, { results: r }) : t ? /* @__PURE__ */ s("div", { ref: a, style: { minHeight: 320 } }) : /* @__PURE__ */ s(x, { c: "dimmed", size: "sm", children: "Loading dashboard table…" });
}
function na(e) {
...
async function ga() {
  try {
    const [e, r] = await Promise.all([
      import("./survey-core-BYbqeafP.js"),
      import("./survey.analytics-CIZLJ_ZF.js")
    ]);
    return {
      Model: e.Model,
      setLicenseKey: e.setLicenseKey,
...
function ba({ results: e }) {
  const r = se(null), [o, a] = j(null), [t, n] = j(!1), [i, c] = j(null);
  return R(() => {
    let d = !1;
    return Promise.all([
      ga(),
  ...
  }, [o, e, i]), t ? /* @__PURE__ */ s(B, { color: "yellow", title: "Charts package not installed", children: /* @__PURE__ */ s(I, { gap: "xs", children: /* @__PURE__ */ u(x, { size: "sm", children: [
    "Install ",
  ...
  ] }) }) }) : i === !1 ? /* @__PURE__ */ s(B, { color: "yellow", title: "SurveyJS Dashboard license not configured", children: /* @__PURE__ */ s(I, { gap: "xs", children: /* @__PURE__ */ u(x, { size: "sm", children: [
...
function fa({ surveyId: e } = {}) {
  const [r, o] = j(null), [a, t] = j(null), [n, i] = j(null), [c, d] = j(!1), [l, g] = j("table"), p = sr({
    pluginId: "sh2-shp-survey-js",
    topic: "surveys/{surveyId}/responses",
    ...
    if (e) {
      ...
      try {
        ...
        o({
          id: b.id,
          surveyId: b.surveyId,
          completedResponses: b.completedResponses,
          totalResponses: b.totalResponses,
          currentVersionRevision: b.currentVersionRevision
        }), t(k);
      } catch (b) {
        i(b.message);
      } finally {
  ...
  return e ? n ? /* @__PURE__ */ s(B, { color: "red", title: "Could not load dashboard", children: n }) : !r || !a ? /* @__PURE__ */ u(M, { gap: "xs", justify: "center", py: "md", children: [
    /* @__PURE__ */ s(ce, { size: "sm" }),
    ...
    /* @__PURE__ */ u(M, { justify: "space-between", children: [
      ...
      /* @__PURE__ */ u(M, { gap: "xs", children: [
        ...
        f !== null && /* @__PURE__ */ u(P, { shadow: "md", withinPortal: !0, children: [
          /* @__PURE__ */ s(P.Target, { children: /* @__PURE__ */ s(J, { leftSection: /* @__PURE__ */ s(Ye, { size: 16 }), variant: "light", children: "Export" }) }),
          /* @__PURE__ */ u(P.Dropdown, { children: [
            /* @__PURE__ */ s(P.Item, { component: "a", href: f.csv, target: "_blank", rel: "noopener noreferrer", children: "CSV" }),
            /* @__PURE__ */ s(P.Item, { component: "a", href: f.xlsx, target: "_blank", rel: "noopener noreferrer", children: "Excel (XLSX)" }),
            /* @__PURE__ */ s(P.Item, { component: "a", href: f.json, target: "_blank", rel: "noopener noreferrer", children: "JSON" })
          ] })
        ] })
      ] })
    ] }),
...
function ma({ surveyId: e, onSurveyChanged: r }) {
  const [o, a] = j(null), [t, n] = j(""), [i, c] = j("default"), [d, l] = j(!1), [g, p] = j(null);
  R(() => {
    let f = !1;
    return xe(e).then((b) => {
      f || (a(b), n(b.name), c(b.themeCode ?? "default"));
    }).catch((b) => p(b.message)), () => {
      f = !0;
    };
  }, [e]);
  const m = async () => {
    if (o) {
      l(!0), p(null);
      try {
        const f = await Be(o.id, {
          name: t.trim(),
          themeCode: i === "default" ? null : i
        }), b = await xe(f.id);
        a(b), n(b.name), c(b.themeCode ?? "default"), r?.(b);
      } catch (f) {
        p(f.message);
      } finally {
        l(!1);
...
function ya({ surveyId: e, onRestored: r } = {}) {
  const [o, a] = j(null), [t, n] = j(null), [i, c] = j(!1), [d, l] = j(null), [g, p] = j({ baseId: null, targetId: null }), [m, f] = j(!1), [b, k] = j(!1), [C, E] = j(null), [v, D] = j(null), S = L(async () => {
    if (e) {
      n(null);
      try {
        const h = await Bo(e);
  ...
  return e ? /* @__PURE__ */ u(I, { gap: "md", children: [
    ...
    /* @__PURE__ */ s(
      Te,
      {
        ...
        children: /* @__PURE__ */ u(I, { gap: "sm", children: [
          ...
          /* @__PURE__ */ u(M, { justify: "flex-end", gap: "xs", children: [
            /* @__PURE__ */ s(J, { variant: "default", onClick: () => l(null), disabled: i, children: "Cancel" }),
            /* @__PURE__ */ s(J, { color: "orange", onClick: () => {
              O();
            }, loading: i, children: "Restore as new revision" })
          ] })
        ] })
      }
    ),
    /* @__PURE__ */ s(
      Te,
      {
        ...
        size: "xl",
        centered: !0,
        children: /* @__PURE__ */ s(I, { gap: "md", children: b ? /* @__PURE__ */ u(M, { justify: "center", py: "md", gap: "xs", children: [
          /* @__PURE__ */ s(ce, { size: "sm" }),
...
function xa() {
  const [{ view: e, surveyId: r }, o] = j(() => Rr()), a = L((n, i) => {
    o((c) => ({
      view: n,
      surveyId: i === void 0 ? c.surveyId : i
    }));
  }, []);
  R(() => {
    const n = () => o(Rr());
    ...
    /* @__PURE__ */ u(
      H,
      {
        ...
        children: [
          ...
          /* @__PURE__ */ s(H.Panel, { value: "list", pt: "md", children: /* @__PURE__ */ s(wa, { onOpen: (n, i) => a(i, n) }) }),
...
function wa({ onOpen: e }) {
  const [r, o] = j(null), [a, t] = j(null), [n, i] = j(!1), [c, d] = j(!1), [l, g] = j(null), [p, m] = j(null), [f, b] = j(""), k = se(null), C = L(async () => {
    t(null);
    try {
      const T = await qr();
      o(T);
  ...
  const E = L((T) => {
    ...
    i(!0);
    try {
      await Be(p, { name: T }), await C(), m(null), b("");
    } catch (_) {
    ...
    /* @__PURE__ */ s(
      Te,
      {
        ...
        children: /* @__PURE__ */ u(I, { gap: "sm", children: [
          ...
          /* @__PURE__ */ u(M, { justify: "flex-end", gap: "xs", children: [
            /* @__PURE__ */ s(J, { variant: "default", onClick: () => g(null), disabled: n, children: "Cancel" }),
            /* @__PURE__ */ s(J, { color: "red", onClick: () => {
              V();
            }, loading: n, children: "Delete" })
          ] })
        ] })
      }
    )
...
function Sa({
  ...
  const [f, b] = j(!1), [k, C] = j(null), E = L((v) => {
    v.preventDefault(), C({ x: v.clientX, y: v.clientY }), b(!0);
  }, []);
  return /* @__PURE__ */ u(y.Tr, { onContextMenu: E, style: { cursor: o ? "text" : "default" }, children: [
    /* @__PURE__ */ u(y.Td, { children: [
      o ? /* @__PURE__ */ u(M, { gap: "xs", wrap: "nowrap", children: [
        ...
        /* @__PURE__ */ s(ls, { onClick: () => n(e.id, "designer"), title: "Open in Designer", children: e.name }),
...
function ka({ position: e, onClose: r, children: o }) {
  return R(() => {
    const a = (n) => {
      const i = n.target;
      i && i.closest("[data-survey-context-menu]") || r();
    }, t = (n) => {
      n.key === "Escape" && r();
    };
...
function Ta({ opened: e, onClose: r, onCreated: o }) {
  const [a, t] = j(""), [n, i] = j(!1), [c, d] = j(null);
  R(() => {
    e || (t(""), d(null), i(!1));
  }, [e]);
  const l = async () => {
  ...
  return /* @__PURE__ */ s(Te, { opened: e, onClose: r, title: "New survey", centered: !0, children: /* @__PURE__ */ u(I, { gap: "sm", children: [
    ...
    /* @__PURE__ */ u(M, { justify: "flex-end", gap: "xs", children: [
      /* @__PURE__ */ s(J, { variant: "default", onClick: r, disabled: n, children: "Cancel" }),
      /* @__PURE__ */ s(J, { onClick: () => {
        l();
      }, loading: n, children: "Create & Open Designer" })
    ] })
  ] }) });
}
function za({
  ...
  const [t, n] = j([]), [i, c] = j(!0), [d, l] = j(null), [g, p] = j("");
  return R(() => {
    let m = !1;
    return c(!0), qr().then((f) => {
      m || (n(f), l(null));
    }).catch((f) => {
      m || l(f.message);
    }).finally(() => {
    ...
    Ar,
    {
      ...
      disabled: a || i,
      error: d ?? void 0,
      nothingFoundMessage: i ? "Loading surveys..." : "No surveys found"
    },
    e
  );
}
const $r = "sh2-shp-survey-js", Ea = "0.2.11", $a = (e) => (Bs(e), rs({
  id: $r,
  version: Ea,
